/*
Based on https://codepen.io/aecend/pen/WbONyK
*/
...
*/
let canvas_width = Math.ceil(window.innerWidth / 10) * 10; //Needs to be a multiple of the resolution value below.
let canvas_height = Math.ceil(window.innerHeight / 10) * 10; //This too.
...
const vec_cells = []; //The array that will contain the grid cells
const particles = []; //The array that will contain the particles
...
function initialize() {
  ...
  canvas = document.getElementById("fluidBg");
  ...
  */
  ...
  // don't start it here, start it in the main script
}
...
function start() {
  ...
  document.addEventListener('mouseleave', reset_mouse_pos);

  // reset all mouse positions when the visibility changes
  document.addEventListener('visibilitychange', reset_mouse_pos, false);
...
function stop() {
  ...
  document.removeEventListener('mouseleave', reset_mouse_pos);

  // reset all mouse positions when the visibility changes
  document.removeEventListener('visibilitychange', reset_mouse_pos, false);
}
...
function draw() {
  ...
  */
  ...
  for (const cell_data of vec_cells) {

    //Loops through all of the rows
    for (const cell_datum of cell_data) {

      // update the cell velocity using the mouse/touch velocity
      change_cell_velocity(cell_datum, mouse_xv, mouse_yv, pen_size);

      //This updates the pressure values for the cell.
      update_pressure(cell_datum);
    }
  }
...
function mouse_move_handler(e) {
  ...
  mouse.x = e.x ?? e.offsetX ?? e.layerX;
  mouse.y = e.y ?? e.offsetY ?? e.layerY;
